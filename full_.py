...
cook=[]
def cookies_condison():
    global cook
    bf=input("Do you want to see cookies (y/n) :")
    if bf in ["Y","y"]:
        cook.append("yes")
    elif bf in ["N","n"]:
        cook.append("no")
...
def apvdef():
    ...
    if key in row:
        ...
        print("[bold black][[bold cyan]$[bold black]] [bold green]Your Key :[bold yellow] "+key)
        print("[bold black][[bold cyan]$[bold black]] [bold violet]Message me for Buy:)")
        
        os.system("xdg-open your_profile_link")
        sys.exit()
